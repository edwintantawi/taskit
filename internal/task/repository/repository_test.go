...
func (s *TaskRepositoryTestSuite) TestFindByID() {
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(d *dependency) {
				d.mockDB.ExpectQuery(regexp.QuoteMeta("SELECT id, user_id, content, description, is_done, due_date, created_at, updated_at FROM tasks WHERE id = $1")).
					WithArgs("task-xxxxx").
					WillReturnError(test.ErrDatabase)
			},
		},
		...
		},
		{
			...
			setup: func(d *dependency) {
				mockRow := sqlmock.NewRows([]string{"id", "user_id", "content", "description", "is_done", "due_date", "created_at", "updated_at"}).
...
func (s *TaskRepositoryTestSuite) TestFindAllByUserID() {
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(d *dependency) {
				d.mockDB.ExpectQuery(regexp.QuoteMeta(`SELECT id, content, description, is_done, due_date, created_at, updated_at FROM tasks WHERE user_id = $1`)).
					WithArgs("user-xxxxx").
					WillReturnError(test.ErrDatabase)
			},
		},
		{
			...
			setup: func(d *dependency) {
				mockRow := sqlmock.NewRows([]string{"id", "content", "description", "is_done", "due_date", "created_at", "updated_at"}).
...
func (s *TaskRepositoryTestSuite) TestUpdate() {
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(d *dependency) {
				d.mockDB.ExpectExec(regexp.QuoteMeta("UPDATE tasks SET content = $2, description = $3, is_done = $4, due_date = $5, updated_at = $6 WHERE id = $1")).
