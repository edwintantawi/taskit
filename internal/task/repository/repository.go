...
func (r *Repository) FindByID(ctx context.Context, taskID entity.TaskID) (entity.Task, error) {
	...
	q := `SELECT id, user_id, content, description, is_done, due_date, created_at, updated_at FROM tasks WHERE id = $1`
	row := r.db.QueryRowContext(ctx, q, taskID)
	err := row.Scan(&task.ID, &task.UserID, &task.Content, &task.Description, &task.IsDone, &task.DueDate, &task.CreatedAt, &task.UpdatedAt)
...
func (r *Repository) FindAllByUserID(ctx context.Context, userID entity.UserID) ([]entity.Task, error) {
	q := `SELECT id, content, description, is_done, due_date, created_at, updated_at FROM tasks WHERE user_id = $1`
	...
	for rows.Next() {
		var task entity.Task
		err := rows.Scan(&task.ID, &task.Content, &task.Description, &task.IsDone, &task.DueDate, &task.CreatedAt, &task.UpdatedAt)
		if err != nil {
			return nil, err
		}
...
func (r *Repository) Update(ctx context.Context, t *entity.Task) (entity.TaskID, error) {
	...
	q := `UPDATE tasks SET content = $2, description = $3, is_done = $4, due_date = $5, updated_at = $6 WHERE id = $1`
	_, err := r.db.ExecContext(ctx, q, t.ID, t.Content, t.Description, t.IsDone, t.DueDate, t.UpdatedAt)
	if err != nil {
		return "", err
	}
	return t.ID, nil
